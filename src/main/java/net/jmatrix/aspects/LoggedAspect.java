...
public class LoggedAspect extends AbstractLoggingAspect
{
   ...
   public Object logged (ProceedingJoinPoint thisJoinPoint, Logged logged) throws Throwable
   {
      ...
      try
      {
         ...
         boolean perTrackedAnnotation = hasPerfTrackedAnnotation(thisJoinPoint);
         if (!perTrackedAnnotation) log.debug("Exiting  "+methodName+"="+format(logged.result(), new Object[] {result}));
         return result;
      }
      catch (Throwable e)
      {
